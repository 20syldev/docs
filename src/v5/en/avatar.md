...
```html
<img src="https://api.sylvain.sh/v5/avatar?seed=alice" alt="Avatar" />
```

With `format=svg`, the response is an inline SVG (`Content-Type: image/svg+xml`):

```html
<img src="https://api.sylvain.sh/v5/avatar?seed=alice&format=svg" alt="Avatar" />
```
...
| `Please provide a valid size (&size={50-2000})` | The `size` parameter is not a valid number       |
